...
with models.DAG(
        config.dag_name('product_metadata_bulk_download_v2'),
        catchup=False,
        max_active_runs=1,
        schedule_interval= None if config.get('schedule_interval')['daily_bulk_download_v2'] == 'None' else config.get('schedule_interval')['daily_bulk_download_v2'],
    ...
    download_product_metadata = rest_api_client(
        ...
        options={
            ...
            'inputPath': '',
            'outputPath': config.get('gs_raw_bucket') + 'source/shopify.batch.bulk.v2_0/report_date={{ds_nodash}}/metadata/',
            'reportDate': '{{ds}}',
    ...
    init_product_metafields = UMGBigQueryOperator(
        ...
        sql='sql/batch/init_product_metafields_bulk_v2.sql',
    ...
    init_product_variant_metafields = UMGBigQueryOperator(
        ...
        sql='sql/batch/init_product_variant_metafields_bulk_v2.sql',
        create_disposition='CREATE_IF_NEEDED',
        use_legacy_sql=False,
        params={'bq_project': config.get('bq_project'), 'bq_dataset': 'shopify_source'},
        dag=dag
    ...
    init_query_response = UMGBigQueryOperator(
        ...
        sql='sql/batch/init_query_response_bulk_v2.sql',
        create_disposition='CREATE_IF_NEEDED',
        use_legacy_sql=False,
        params={'bq_project': config.get('bq_project'), 'bq_dataset': 'shopify_source'},
        dag=dag
    ...
    load_to_bq_product_metafields = BashOperator(
        ...
        bash_command="""
                    ...
                    bq load --ignore_unknown_values --replace --source_format=NEWLINE_DELIMITED_JSON {bq_project}:shopify_source.product_metafields_batch_bulk_v2\${date} {gs_raw_bucket}source/shopify.batch.bulk.v2_0/report_date={date}/metadata/product_metadata/*
    ...
    load_to_bq_product_variant_metafields = BashOperator(
        ...
        bash_command="""
                    ...
                    bq load --ignore_unknown_values --replace --source_format=NEWLINE_DELIMITED_JSON {bq_project}:shopify_source.product_variant_metafields_batch_bulk_v2\${date} {gs_raw_bucket}source/shopify.batch.bulk.v2_0/report_date={date}/metadata/product_variant_metadata/*
                    """.format(bq_project=config.get('bq_project'),
                               dag_dir=config.dag_dir(),
                               date='{{ds_nodash}}',
                               gs_raw_bucket=config.get('gs_raw_bucket'),
    ...
    load_to_bq_query_response = BashOperator(
        ...
        bash_command="""
                    ...
                    bq load --ignore_unknown_values --replace --source_format=NEWLINE_DELIMITED_JSON {bq_project}:shopify_source.bulk_query_response_v2\${date} {gs_raw_bucket}source/shopify.batch.bulk.v2_0/report_date={date}/metadata/query_status_response/*
                    """.format(bq_project=config.get('bq_project'),
                               dag_dir=config.dag_dir(),
                               date='{{ds_nodash}}',
                               gs_raw_bucket=config.get('gs_raw_bucket'),
