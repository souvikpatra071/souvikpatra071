...
shopify_sale_input_path=config.get('shopify_ecomm_sale_input_path_v2')
shopify_sale_output_path=config.get('shopify_ecomm_sale_output_path_v2')
...
with models.DAG(
        config.dag_name('shopify_ecomm_sale_hourly_all'),
        catchup=False,
        ...
        insert_source_data_temp = UMGBigQueryOperator(
            ...
            sql='sql/input_ecomm_shopify_hourly.sql',
            destination_dataset_table= bq_source+ '.' +temp_dataset+'.shopify_ecomm_input_temp_{{ds_nodash}}',
        ...
        insert_source_data_main = UMGBigQueryOperator(
            ...
            sql='''
                ...
                FROM `{{params.bq_source}}.{{params.temp_dataset}}.shopify_ecomm_input_temp_{{ds_nodash}}` s
            ...
            destination_dataset_table = bq_source+ '.' +temp_dataset+'.shopify_ecomm_input_{{ds_nodash}}',
            write_disposition='WRITE_TRUNCATE',
            allow_large_results=True,
            time_partitioning={
                'type':'DAY',
        ...
        export_to_gcs = BashOperator(
            ...
            bash_command = 'bq extract --destination_format NEWLINE_DELIMITED_JSON {bq_source_project}:{data_provider}.{table} {input_path}'.format(
                ...
                table='shopify_ecomm_input_{{ds_nodash}}',
        ...
        job_class='com.umusic.datalake.elt.shopify.ingestion.dataflow.ShopifyEcommSalesTransform',
        options={
            ...
            'numWorkers': '5',
            'maxNumWorkers': '100',
            'productQuery': "select distinct id from `{project}.{dataset}.{table}` where lower(product_type) like '%digital%'".format(project=config.get('audience_project'), dataset=config.get('shopify_unified_dataset'), table=config.get('bq_shopify_product_table'))
        },
        dag=dag)
        ...
        load_to_bq = BashOperator(
            ...
            bash_command="""
                    bq load --replace --max_bad_records=25 --source_format=NEWLINE_DELIMITED_JSON {bq_project}:{bq_dataset}.{bq_table} {shopify_sale_output_path}report_date={datetime}/* {base_folder}/json/ecomm_schema.json
        ...
        load_reporting_shopify_sales = UMGBigQueryOperator(
            task_id=config.task_name('load_reporting_sales'),
            bigquery_conn_id='google_cloud_default',
            use_legacy_sql=False,
            sql='sql/report/load_reporting_shopify_sales.sql',
            params={'bq_project': config.get('bq_project'),'bq_source_dataset': config.get('bq_shopify_dataset')},
            destination_dataset_table='{project}.{dataset}.{table}'.format(project=bq_source_project,
                                                                        dataset=config.get('bq_shopify_dataset'), table=config.get('bq_shopify_sales_table')),
            write_disposition='WRITE_TRUNCATE',
            allow_large_results=True,
            dag=dag)

        finish = on_job_finish(subscribers=config.get('notification_email'),
                           variables={"service_impacted": "Shopify Ecomm Sale hourly"},
                           dag=dag)

        insert_source_data_temp >> insert_source_data_main >> export_to_gcs >> shopify_sale_transform >> load_to_bq >> load_reporting_shopify_sales >> finish
